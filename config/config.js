...
// --- Render / small-VM overrides --------------------------------------------
// When deployed to Render (or any host that sets $RENDER), fold all worker
// processes into the main process. With only 512 MB RAM and 0.1 CPU on the
// free tier, spawning network+sim+validator+... children would OOM instantly.
if (process.env.RENDER || process.env.KOYEB_APP_NAME || process.env.FLY_APP_NAME) {
